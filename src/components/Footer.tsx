import { Youtube, Instagram } from "lucide-react";

const Footer = () => {
  return (
    <footer className="bg-primary text-primary-foreground py-8 px-4 mt-12">
      <div className="container mx-auto">
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-6">
            <a
              href="https://www.youtube.com/@helenaexplora"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:opacity-80 transition-opacity flex items-center gap-2"
              aria-label="YouTube"
            >
              <Youtube className="w-6 h-6" />
              <span className="text-sm">YouTube</span>
            </a>
            <a
              href="https://www.instagram.com/helenaexplora_usa"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:opacity-80 transition-opacity flex items-center gap-2"
              aria-label="Instagram"
            >
              <Instagram className="w-6 h-6" />
              <span className="text-sm">Instagram</span>
            </a>
            <a
              href="https://www.tiktok.com/@helenaexplora"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:opacity-80 transition-opacity flex items-center gap-2"
              aria-label="TikTok"
            >
              <svg
                className="w-5 h-5"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" />
              </svg>
              <span className="text-sm">TikTok</span>
            </a>
          </div>
          
          <p className="text-sm opacity-80 text-center">
            © {new Date().getFullYear()} Helena Explora. Compartilhando experiências sobre estudar nos EUA.
          </p>
          
          <p className="text-xs opacity-60 text-center max-w-md">
            As informações compartilhadas são baseadas em experiências pessoais e não substituem 
            aconselhamento profissional ou legal.
          </p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
